...
class RailsDependencyPrunerTest < Minitest::Test
  ...
  def test_verify_production_requires_request_measurement_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_request_measurement_verify") do |dir|
      ...
      payload = JSON.parse(stdout)
      assert_includes payload.fetch("errors"), "production verify missing high-risk transform proof: disable_eager_load requires measurement.target=requests"
      assert_equal [
        {
          "transform_id" => "disable_eager_load",
          "requirement" => "request_measurement",
          "missing_requirements" => ["measurement.target=requests"],
        },
      ], payload.dig("production_risks", "high_risk_transform_gaps")
    end
  end
  ...
  def test_verify_production_requires_declared_mailbox_coverage_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_mailbox_verify") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      ...
      payload = JSON.parse(stdout)
      assert_includes payload.fetch("errors"), "production verify missing high-risk transform proof: disable_eager_load requires inbound_email.ApplicationMailbox"
      assert_equal(
        {
          "transform_id" => "disable_eager_load",
          "requirement" => "declared_workload_coverage",
          "missing_requirements" => ["inbound_email.ApplicationMailbox"],
        },
        payload.dig("production_risks", "high_risk_transform_gaps", 0),
      )
    end
  ...
  def test_verify_production_allows_disable_eager_load_with_declared_mailbox_coverage
    Dir.mktmpdir("rails_dependency_pruner_eager_load_mailbox_pass") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      FileUtils.mkdir_p(File.join(app_root, "app/mailboxes"))
      File.write(File.join(app_root, "app/mailboxes/application_mailbox.rb"), <<~RUBY)
  ...
  def test_verify_production_requires_declared_action_text_coverage_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_action_text_verify") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      File.write(File.join(app_root, "app/models/avatar.rb"), <<~RUBY)
        class Avatar < ApplicationRecord
      ...
      payload = JSON.parse(stdout)
      assert_includes payload.fetch("errors"), "production verify missing high-risk transform proof: disable_eager_load requires action_text.Avatar#bio"
      assert_equal(
        {
          "transform_id" => "disable_eager_load",
          "requirement" => "declared_workload_coverage",
          "missing_requirements" => ["action_text.Avatar#bio"],
        },
        payload.dig("production_risks", "high_risk_transform_gaps", 0),
      )
    end
  ...
  def test_verify_production_allows_disable_eager_load_with_declared_action_text_coverage
    Dir.mktmpdir("rails_dependency_pruner_eager_load_action_text_pass") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      File.write(File.join(app_root, "app/models/avatar.rb"), <<~RUBY)
        class Avatar < ApplicationRecord
  ...
  def test_verify_production_requires_declared_active_storage_coverage_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_attachment_verify") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      File.write(File.join(app_root, "app/models/avatar.rb"), <<~RUBY)
        class Avatar < ApplicationRecord
      ...
      payload = JSON.parse(stdout)
      assert_includes payload.fetch("errors"), "production verify missing high-risk transform proof: disable_eager_load requires attachments.Avatar#image"
      assert_equal(
        {
          "transform_id" => "disable_eager_load",
          "requirement" => "declared_workload_coverage",
          "missing_requirements" => ["attachments.Avatar#image"],
        },
        payload.dig("production_risks", "high_risk_transform_gaps", 0),
      )
    end
  ...
  def test_verify_production_allows_disable_eager_load_with_declared_active_storage_coverage
    Dir.mktmpdir("rails_dependency_pruner_eager_load_attachment_pass") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      File.write(File.join(app_root, "app/models/avatar.rb"), <<~RUBY)
        class Avatar < ApplicationRecord
  ...
  def test_verify_production_requires_declared_rake_task_coverage_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_rake_task_verify") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
  ...
  def test_verify_production_allows_disable_eager_load_with_declared_rake_task_coverage
    Dir.mktmpdir("rails_dependency_pruner_eager_load_rake_task_pass") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      FileUtils.mkdir_p(File.join(app_root, "lib/tasks"))
      File.write(File.join(app_root, "lib/tasks/maintenance.rake"), <<~RAKE)
  ...
  def test_verify_production_requires_mounted_app_request_coverage_for_disable_eager_load
    Dir.mktmpdir("rails_dependency_pruner_eager_load_mount_verify") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      ...
      payload = JSON.parse(stdout)
      assert_includes payload.fetch("errors"), "production verify missing high-risk transform proof: disable_eager_load requires requests.GET /admin"
      assert_equal(
        {
          "transform_id" => "disable_eager_load",
          "requirement" => "declared_workload_coverage",
          "missing_requirements" => ["requests.GET /admin"],
        },
        payload.dig("production_risks", "high_risk_transform_gaps", 0),
      )
    end
  ...
  def test_verify_production_allows_disable_eager_load_with_mounted_app_request_coverage
    Dir.mktmpdir("rails_dependency_pruner_eager_load_mount_pass") do |dir|
      app_root = File.join(dir, "app")
      FileUtils.cp_r(FAKE_APP_ROOT, app_root)
      File.write(File.join(app_root, "config/routes.rb"), <<~RUBY)
        Rails.application.routes.draw do
  ...
  def test_coverage_manifest_normalizes_reviewed_request_entries
    Dir.mktmpdir("rails_dependency_pruner_request_coverage_review") do |dir|
      ...
      manifest = RailsDependencyPruner::CoverageManifest.load(manifest_path)
      assert_empty manifest.request_entries
      assert_equal false, manifest.request_covered?(method: "GET", path: "/admin")
      ...
      assert_equal(
        ...
        manifest.request_entries,
      )
      assert_equal true, manifest.request_covered?(method: "GET", path: "/admin")
      assert_equal true, manifest.request_covered?(method: "POST", path: "/health")
      assert_equal false, manifest.request_covered?(method: "POST", path: "/admin")
      ...
      manifest = RailsDependencyPruner::CoverageManifest.load(manifest_path)
      assert_equal true, manifest.request_covered?(method: "GET", path: "/privacy")
      assert_equal true, manifest.request_covered?(method: "DELETE", path: "/status")
    end
  end
