...
request, p95, and p99 latency, and reviewed exact coverage for app-declared job
classes, mailer actions, and channel classes. It also requires exact inbound
email mailbox coverage, Active Storage attachment declaration and action
coverage, exact Action Text rich-text declaration coverage, exact rake task
coverage, and mounted Rack app or engine paths. RSS savings alone are not
